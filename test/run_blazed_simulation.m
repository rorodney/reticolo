...
grating = build_grating('trapezoidal', period_nm, depth_nm ,ratio_groove_width_to_groove_period, trapezoid_angle, x_res_nm);
...
sweep.alpha_deg = 86;      % only used if sweep.type is 'energy' and no Cff specified
...
title_str = sprintf('%s | %d l/mm | %s | %s | %s | Order %+d', ...
    grating.type, grPeriod_lpermm, layer_str, inc_tag, pol_str, options.GR_Order);
...
png_name = sprintf('rcwa_%s_%dlmm_%s_%s_order%+d.png', ...
    grating.type, grPeriod_lpermm, inc_tag, pol_str, options.GR_Order);
saveas(gcf, png_name);
